...
- The instability follow-up evaluated 8 and 32 windows for pretrained sine and ETTm1 adapters. The 32-window error correlations were `-0.463/0.183` on sine and `0.119/0.051` on ETTm1 across seeds. The initial `0.794` sine correlation on 8 windows did not replicate, so raw local text sensitivity is not reliable enough to drive perturbation weighting; this negative result is recorded in `docs/instability_results.md`.
- The first lightweight mitigation injects normalized history noise with standard deviation `0.02` during teacher forcing. On ETTm1, mean RMSE changed from `0.11080` to `0.10571` across seeds 7 and 17, but spectral amplitude MAE worsened from `0.23757` to `0.25069` and seed 17 showed no time-domain gain. This remains an exploratory trade-off, documented in `docs/mitigation_results.md`, rather than a confirmed method improvement.
