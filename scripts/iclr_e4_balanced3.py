"""ICLR P1-8: 3-class balanced OOD (trend / periodic / local-oracle).

Adds a local-oracle unseen composition (AR(1) + weak periodic) to the balanced
OOD test, so the balanced set covers all THREE experts.
"""
...
def main():
    ...
    ap.add_argument("--out", type=Path, default=Path("results/iclr/e4_balanced3/qwen3_8b_official.json"))
    a=ap.parse_args()
    seeds=[int(s) for s in a.seeds.split(",")]
    experts=[TrendExpert(),PeriodicExpert(),LocalExpert()]
    wdir=Path("results/iclr/e4_balanced3/windows"); wdir.mkdir(parents=True,exist_ok=True)
    ...
    for s in seeds:
        ...
        if wpath.is_file():
            ...
            for kind in ["mixture","regime"]:
                for _ in range(4000):
                    ...
                    if j<2:
                        ctx.append(cw); fut.append(fw); oracle.append(j)
            # class 2 from AR_weak_sine (local-oracle)
            for _ in range(6000):
                cw,fw=gen_ar_weak_sine(rng)
                errs=[float(np.mean((e.predict(cw,16)-fw)**2)) for e in experts]
                if int(np.argmin(errs))==2:
                    ctx.append(cw); fut.append(fw); oracle.append(2)
        ...
        print(f"[seed {s}] balanced3 {len(ctx)} windows (T/P/L={int((oracle==0).sum())}/{int((oracle==1).sum())}/{int((oracle==2).sum())})")
        ...
        print(f"    qwen balacc={row['qwen']['balanced_acc']:.3f} mse={row['qwen']['mse']:.3f} | mlp={row['mlp']['balanced_acc']:.3f} const={row['constant_trend']['mse']:.3f}")
