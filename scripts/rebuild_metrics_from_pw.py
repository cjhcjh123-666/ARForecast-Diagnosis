...
rows = []
for pw in sorted((OUT/"perwindow").glob("pw_*.npz")):
    parts = pw.stem[len("pw_"):].rsplit("_s", 1)
    model_init = parts[0]; seed = int(parts[1])
    # model_init ends with _pretrained or _random
    assert model_init.endswith("_pretrained") or model_init.endswith("_random"), model_init
    init = "pretrained" if model_init.endswith("_pretrained") else "random"
    model = model_init[: -len("_" + init)]
    z = np.load(pw)
    rec = {"model": model, "init": init, "seed": seed}
    # A orig
    if "a_pred" in z and "a_true" in z:
        rows.append({**rec, "task": "A_recog_orig", "test_set": "synth5", "n_test": len(z["a_pred"]),
                     "accuracy": round(float(np.mean(z["a_pred"] == z["a_true"])), 4)})
    if "as_pred" in z and "as_true" in z:
    ...
    for tag in ["bal3", "bal3n"]:
        o = f"c_{tag}_oracle"; pr = f"c_{tag}_pred"; er = f"c_{tag}_err"
        if o in z and pr in z and er in z:
            ba, mf1, r3 = recog_metrics3(z[pr], z[o])
            mse = float(np.mean(z[er][np.arange(len(z[pr])), z[pr]]))
            rows.append({**rec, "task": "C_routing", "test_set": f"{tag}_s{seed}", "n_test": len(z[pr]),
