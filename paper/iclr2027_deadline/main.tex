...
separate three questions that end-to-end scores often conflate: whether
pretraining helps a forecasting model, which temporal properties are linearly
accessible in frozen representations, and whether a native question-answering
interface can use those properties.  In matched forecasting experiments,
pretrained initialization improves the trained forecaster.  Across ten frozen
backbones, paired with same-architecture random controls over three seeds, the
largest consistent final-layer gaps occur for dominant period (balanced
accuracy $0.715$ vs. $0.364$) and period regression (NMAE $0.200$ vs. $0.594$);
anomaly presence, local dependence, and noise level also improve, while trend
strength is mixed across models.  These probes establish accessibility, not
model understanding or successful use by another interface.  In corrected
shared-head IRTS diagnostics, the first two models show small, inconsistent
pretrained--random differences.  In a corrected native run, official
Qwen3-8B-Base obtains 0.153 full-prompt accuracy with 0.473 parse coverage;
condition-dependent parse coverage prevents a simple language-prior
interpretation.  Controlled generation and routing experiments further show
that recognition, numeric continuation, and downstream use can separate.
The resulting evidence supports a selective
temporal-structure prior rather than a blanket claim that language models
understand or solve time-series tasks.
\end{abstract}
...
Language pretraining makes selected temporal properties more linearly
accessible; numeric generation and native temporal QA can remain weak.}}
\end{center}

Concretely, we make the following contributions.
\begin{itemize}
  \item \textbf{Controlled initialization evidence for pretraining benefit (E1).}
  ...
  artifact: with full fine-tuning on Qwen3-0.6B, pretrained initialization is
  better in all three runs, although the random control is highly variable
  (ratio of mean MSEs $20.9\times$; one seed is much closer).
  \item \textbf{A multi-model map of accessible temporal properties.}
  We pair pretrained and random final-layer representations for ten language
  models and three seeds.  Period, anomaly presence, local dependence, and
  noise level show cross-model gains, whereas trend-strength regression is
  mixed.  We report all valid tasks, including weak and negative results.
  ...
  MSE than the oracle expert.  On the official 8B checkpoint, free-form outputs
  parse on about $99\%$ of 600 evaluated windows while error remains
  $3.6$--$4.4\times$ the oracle, separating numeric error from parse failure.
  ...
  \item \textbf{A boundary test through temporal QA.}  We distinguish native
  zero-shot IRTS accuracy from a trained-head diagnostic and isolate question
  and temporal branches.  The corrected one-model native result is weak and
  parse-sensitive, so we do not treat probe accessibility as QA ability.
\end{itemize}
...
manifold.  Our experiments compare recognition and generation measurements
without treating their association as proof of a shared causal mechanism.
...
\paragraph{Models.}  Qwen3-0.6B / 1.7B and official Qwen3-8B-Base checkpoints.
Random controls use the identical architecture with re-initialized weights.
...
Table~\ref{tab:scale} shows recognition accuracy and generation reliability
across the evaluated model sizes.  Frozen recognition is high at every scale
and above the matched random controls.  Native numeric generation remains
several times worse than the oracle expert even when parse coverage is nearly
complete.  The official Qwen3-8B-Base checkpoint replaces the earlier custom
8B checkpoint in the reported 8B row.
...
\toprule
Model & Family & Recog. (pre.) & Recog. (rand.) & Gen. parse & Gen. / oracle \\
\midrule
DistilGPT2 & GPT & $0.973\pm0.009$ & $0.744\pm0.022$ & $1.000$ & $15.4\times$ \\
GPT-2 (124M) & GPT & $0.980\pm0.008$ & $0.699\pm0.041$ & $1.000$ & $12.4\times$ \\
Qwen3-0.6B & Qwen & $0.984\pm0.006$ & $0.789\pm0.010$ & $0.980$ & $9.9\times$ \\
Qwen3-1.7B & Qwen & $0.979\pm0.011$ & $0.837\pm0.012$ & $0.575$ & $8.1\times$ \\
Qwen3-8B-Base & Qwen & $0.981\pm0.003$ & $0.904\pm0.025$ & $0.990$ & $4.0\times$ \\
\bottomrule
\end{tabular}
}
\caption{R5 family $\times$ scale: recognition and generation (3 seeds,
...
appears in both evaluated families: GPT-2 and DistilGPT2 have a larger
pretrained--random recognition gap ($97$--$98\%$ vs.\ $70$--$74\%$ random) and
the largest generation gaps ($12$--$15\times$ oracle despite $100\%$ parse
rate).  This cross-family observation broadens the evidence but does not by
itself identify a shared causal mechanism.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\linewidth]{figs/fig3_layerwise.pdf}
...
coverage varies for the smaller checkpoints and MSE remains well above the
oracle expert.  The official 8B checkpoint parses nearly every output, yet its
error is still about four times the oracle on a larger 600-window evaluation
(Table~\ref{tab:e3b}).
...
Qwen3-8B-Base & $0.98$ & $1.00$ & $1.47$ & $0.34$ & $4.4\times$ \\
\bottomrule
\end{tabular}
...
\begin{tabular}{lccc}
\toprule
Seed & Windows & Parse rate & MSE / oracle \\
\midrule
7 & 200 & $0.985$ & $3.60\times$ \\
17 & 200 & $0.995$ & $4.10\times$ \\
27 & 200 & $0.990$ & $4.42\times$ \\
\bottomrule
\end{tabular}
\caption{E3b: official Qwen3-8B-Base free-form generation on an expanded
evaluation.  Near-complete parsing does not remove the numeric error gap.}
\label{tab:e3b}
\end{table}

\paragraph{The error gap is stable to numeric precision.}
Table~\ref{tab:genrob} varies the serialization precision on 100 official-8B
windows.  Parse coverage remains $98$--$100\%$ and the error ratio remains
$4.1$--$4.6\times$.  The result isolates this particular formatting choice;
it does not rule out every possible prompting or decoding protocol.
...
\begin{tabular}{lcc}
\toprule
Decimal places & Parse & MSE/oracle \\
\midrule
1 & $0.990$ & $4.59\times$ \\
2 & $1.000$ & $4.14\times$ \\
3 & $0.980$ & $4.20\times$ \\
\bottomrule
\end{tabular}
\caption{E3c: official Qwen3-8B-Base serialization-precision control.}
\label{tab:genrob}
\end{table}
...
\input{verified_diagnostics.tex}
...
(\emph{trend-like}, \emph{periodic}, \emph{noise-dominated}).  In our evaluated
checkpoints, such distinctions are more linearly separable after language
pretraining and sometimes transfer to OOD dynamics.  In contrast, generating
...
$\gamma>0$ at the linear probe; generation error can grow as $\rho^H$ in
horizon $H$.  This sketch is consistent with an asymmetry when $\rho^H$
dominates the classification margin, including the horizon sweep in the
appendix, but it does not identify the mechanism.  It also predicts \emph{when}
...
real-data routing inherits this simplicity.  Model families and sizes differ
in more than parameter count, so the scale table is descriptive rather than a
scaling-law estimate.  The M4 result shows that recognition transfer does not hold
universally at small scale.  Linear probes measure accessibility under a
specific readout and do not establish semantic understanding or use by a
generative interface.  The current corrected IRTS trained-head result covers
one task type, two models, and one seed; corrected native zero-shot coverage is
one model and one seed.  Finally, we study frozen recognition;
how recognition evolves during fine-tuning is analyzed in the appendix but
...
pretraining helps the trained forecaster and makes several temporal properties
more accessible to linear readouts, but the effect is selective across
properties and interfaces.  Frozen numeric generation remains fragile, while
the corrected trained-head temporal QA diagnostic shows no consistent gain and
the corrected one-model native result is weak and parse-sensitive.  These separations motivate
measuring representation, interface use, and forecasting behavior independently
rather than treating any one of them as evidence for the others.
