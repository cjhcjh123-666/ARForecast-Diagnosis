...
\item \textbf{Patch-level dynamic routing} (Sec.~\ref{sec:e6}): routing each
  horizon segment independently adapts to within-window dynamics changes
  ($-33\%$ MSE on regime-switch windows), while stationary windows do not
  benefit --- the temporal-structure prior is most valuable under heterogeneous
  dynamics.
\end{itemize}
...
\subsection{Patch-level dynamic routing}
\label{sec:e6}
Finally we test the dynamic version of the router: the horizon is split into
four 4-step segments, and each segment is routed independently by the frozen
LLM probe applied to the 32 steps immediately before it (the probe is trained
on 32-step clean-dynamics patches).  Table~\ref{tab:patch} reports per-kind
forecast MSE.  Dynamic routing helps exactly where dynamics change within a
window: on \emph{regime} windows it cuts MSE from 0.82 to 0.55 (toward the
per-segment oracle upper bound of 0.28), and on \emph{local} windows from 1.64
to 1.31.  It does not help stationary windows (\emph{periodic} 0.28 $\to$ 1.00;
\emph{trend} 0.001 $\to$ 0.065), where 32-step patches carry weaker structural
evidence and routing errors compound.  The LLM patch router still beats the
hand-feature patch router by 34\% overall (0.76 vs.\ 1.14), confirming that the
temporal-structure prior is most valuable when dynamics are heterogeneous ---
across windows (Sec.~\ref{sec:e4}) or within them.

\begin{table}[t]
\caption{Patch-level dynamic routing: per-kind forecast MSE.}
\label{tab:patch}
\centering
\begin{tabular}{lccc}
\toprule
...
\midrule
regime  & 0.817 & \textbf{0.546} & 0.280\\
local   & 1.635 & \textbf{1.305} & 0.624\\
...
synthetic probe data (three seeds); clean stationary dynamics are easy for cheap
features, so the LLM's advantage is specifically under distribution shift, and
patch-level dynamic routing helps only where dynamics change within a window.
Future work extends dynamic routing to longer horizons and real multivariate
data.
